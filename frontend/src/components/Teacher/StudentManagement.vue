...
const props = defineProps<{
  ...
  academicYearOptions?: Array<{id: number, name: string, is_active?: boolean}>;
}>();

// Emit for updating academic year
...
// Computed for academic year options (use prop or default)
const academicYearOptionsLocal = computed(() => {
  return props.academicYearOptions || [];
});

// Handle academic year change
const onAcademicYearChange = () => {
  emit('update:academicYearId', selectedAcademicYearIdLocal.value);
};
  ...
  students.value.filter((s) => {
    const matchesSearch =
      ...
      String(s.student_code || "")
        ...
        .includes(searchQuery.value.toLowerCase());
    const matchesClass =
      classFilter.value === "All Classes" || s.class === classFilter.value;
...
const roleOf = (student: Student | null) =>
...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-slate-500 font-medium">
          View and manage student identification
        </p>
      </div>
      <button
        class="px-4 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-bold"
        :disabled="loading"
        @click="loadStudents"
      >
        Refresh
    ...
    >
      <div
        class="p-4 border-b border-slate-200 bg-slate-50/50 flex flex-wrap items-center justify-between gap-4"
      >
        <div class="flex flex-wrap items-center gap-3 flex-1">
          ...
          <select
            v-model="selectedAcademicYearIdLocal"
      ...
      >
        ...
        >
          <div class="flex flex-col items-center text-center space-y-4">
            <div
              class="size-24 rounded-full border-4 border-slate-50 p-1 bg-white shadow-inner overflow-hidden"
            >
              <img
                v-if="s.avatar"
                :src="s.avatar"
            ...
            <div class="flex items-center gap-2 w-full">
              <div
                class="flex-1 px-3 py-1.5 bg-slate-50 rounded-lg text-[10px] font-bold text-slate-600 border border-slate-100"
              >
                CLASS {{ s.class || "-" }}
              </div>
              <button
    ...
    >
      <div v-if="selectedStudent" class="space-y-6 flex flex-col items-center">
        <div
          class="w-[280px] h-[400px] bg-white rounded-xl shadow-2xl overflow-hidden relative border border-slate-200 flex flex-col items-center p-6 text-slate-900 shrink-0"
        >
          <svg
          ...
          >
            <path
              d="M0 20 L20 20 L30 10 M80 0 L80 20 L100 40 M0 80 L20 80 L40 100"
              stroke="currentColor"
              fill="none"
              strokeWidth="0.5"
            />
          ...
          <div class="w-full relative flex items-start z-10">
            <div class="flex flex-col items-center">
              <div
                class="size-10 bg-slate-900 rounded-full flex items-center justify-center text-white font-black text-[12px] tracking-tight relative"
              >
                PNC
                <div
                  class="absolute -bottom-1 left-1/2 -translate-x-1/2 w-6 h-1 bg-sky-500"
                ></div>
              </div>
            </div>
            <p
              class="absolute left-1/2 -translate-x-1/2 text-xl font-black text-slate-800 tracking-tighter"
            >
          ...
          <div class="mt-8 relative z-10">
            <div
              class="size-32 rounded-full border-4 border-sky-400 p-1 bg-white overflow-hidden"
            >
              <div
                class="w-full h-full rounded-full overflow-hidden bg-slate-100"
              >
                <img
            ...
            <div
              class="absolute -inset-2 border-t-4 border-l-4 border-sky-500 rounded-full opacity-50"
            ></div>
          </div>
          <div class="mt-8 text-center z-10 space-y-1">
            <h3 class="text-2xl font-black text-sky-900 leading-tight">
              {{ selectedStudent.name }}
            </h3>
            <p
              class="text-[10px] font-bold text-slate-500 tracking-[0.2em] uppercase"
            >
              {{ roleOf(selectedStudent) }}
            </p>
          </div>
          <div class="mt-auto z-10 pb-2">
            <p class="text-base font-bold text-slate-800 tracking-tight">
              ID NB:
              <span class="font-black">{{ selectedStudent.student_code }}</span>
            </p>
          </div>
        </div>
        <button
          class="w-full flex items-center justify-center gap-2 py-3 bg-slate-900 text-white rounded-xl font-bold text-sm shadow-xl hover:bg-slate-800 transition-all"
        >
