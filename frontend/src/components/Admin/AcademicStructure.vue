...
const studentsToAssign = [
  { name: 'Alice Wonder', id: 'S-2001' },
  { name: 'Bob Builder', id: 'S-2002' },
  { name: 'Charlie Brown', id: 'S-2003' },
];
...
<template>
  <div class="space-y-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
        ...
        <div class="flex -space-x-2">
          <div
            v-for="(c, i) in classes.slice(0, 4)"
            :key="i"
            class="size-8 rounded-full bg-slate-200 border-2 border-white flex items-center justify-center text-[10px] font-bold text-slate-600"
          >
            {{ c.name.split(' ')[1] }}
          </div>
          <div
            v-if="classes.length > 4"
            class="size-8 rounded-full bg-slate-100 border-2 border-white flex items-center justify-center text-[10px] font-bold text-slate-400"
          >
            +{{ classes.length - 4 }}
          </div>
        </div>
        ...
        <tbody class="divide-y divide-slate-100">
          <tr v-for="(c, i) in filteredClasses" :key="i" class="hover:bg-slate-50 transition-colors">
            ...
            <td class="px-6 py-4 text-right">
              <button @click="handleEditClass(c)" class="text-xs font-bold text-primary hover:underline">Edit</button>
            </td>
          </tr>
    ...
    <Modal :is-open="isClassModalOpen" title="Create New Class" @close="isClassModalOpen = false">
      <div class="space-y-4">
        <div class="space-y-1">
    ...
    <Modal
      :is-open="isEditClassModalOpen"
      :title="`Edit ${selectedClass?.name ?? ''}`"
      @close="isEditClassModalOpen = false"
    >
      <div class="space-y-4">
        <div class="space-y-1">
          <label class="text-[10px] font-bold text-slate-500 uppercase">Class Name</label>
    ...
    <Modal :is-open="isSessionModalOpen" title="Manage Academic Sessions" @close="isSessionModalOpen = false">
      <div class="space-y-4">
    ...
    <Modal :is-open="isAllClassesModalOpen" title="All Classes" size="lg" @close="isAllClassesModalOpen = false">
      <div class="space-y-4">
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
          <div
            v-for="(c, i) in classes"
            :key="i"
            class="p-4 bg-slate-50 border border-slate-200 rounded-xl hover:border-primary transition-colors cursor-pointer"
          >
    ...
    >
      <div class="space-y-4">
        ...
        <div class="max-h-60 overflow-y-auto border border-slate-200 rounded-xl divide-y divide-slate-100">
          <div v-for="(s, i) in studentsToAssign" :key="i" class="p-3 flex items-center justify-between hover:bg-slate-50">
            <div>
    ...
    <Modal
      :is-open="isPromoteModalOpen"
      title="Promote Students to Next Year"
      @close="isPromoteModalOpen = false"
    >
      <div class="space-y-4">
        ...
        <div class="space-y-1">
          <label class="text-[10px] font-bold text-slate-500 uppercase">Target Grade</label>
          <input
            type="text"
            class="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm outline-none"
            value="Grade 11"
            readonly
          />
        </div>
