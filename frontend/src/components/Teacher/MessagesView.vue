...
<template>
  <div
    class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex h-[calc(100vh-180px)]"
  >
    <div class="w-96 border-r border-slate-100 flex flex-col bg-slate-50/30">
      <div class="p-6 border-b border-slate-100 bg-white">
        <div class="flex items-center justify-between mb-4">
          ...
          <span
            class="px-2 py-1 bg-primary/10 text-primary text-[10px] font-black rounded-lg uppercase"
          >
      ...
      <div class="flex-1 overflow-y-auto">
        ...
        <template v-else-if="filteredJustifications.length > 0">
          ...
          >
            ...
            <div
              v-else
              class="size-12 rounded-2xl bg-slate-100 border border-slate-200 shrink-0"
            />
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mb-1">
                <span
                  ...
                  >{{ j.classCode }}</span
                >
                <span v-if="j.sessionName" class="text-[10px] text-slate-400"
                  >|</span
                >
                <span
                  v-if="j.sessionName"
        ...
        <div v-else class="p-12 text-center space-y-3">
          <div
            class="size-16 bg-slate-100 rounded-3xl flex items-center justify-center mx-auto text-slate-300"
          >
            <ShieldCheck :size="32" />
          </div>
    ...
    <div class="flex-1 flex flex-col bg-white">
      <p
        v-if="errorMessage"
        class="m-6 p-3 rounded-lg bg-rose-50 text-rose-700 text-sm"
      >
        {{ errorMessage }}
      </p>
      <div v-if="activeJustification" class="flex-1 flex flex-col">
        ...
        >
          <div class="flex items-center gap-5">
            <img
              v-if="activeJustification.studentPhoto"
              :src="activeJustification.studentPhoto"
              alt=""
            ...
            <div
              v-else
              class="size-16 rounded-[2rem] bg-slate-100 border-4 border-slate-50 shadow-sm"
            />
            <div>
              ...
              <div class="flex items-center gap-3 mt-1">
                <span
                  class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-black rounded uppercase"
                  >ID: {{ activeJustification.studentId }}</span
                >
                <span
                  class="px-2 py-0.5 bg-primary/10 text-primary text-[10px] font-black rounded uppercase"
                  >{{ activeJustification.classCode }}</span
                >
                <span
                  v-if="activeJustification.sessionName"
                  class="px-2 py-0.5 bg-slate-100 text-slate-500 text-[10px] font-black rounded uppercase"
                >
                  {{ activeJustification.sessionName }}
                </span>
                <span
                  :class="[
                    'px-2 py-0.5 text-[10px] font-black rounded uppercase',
                    activeJustification.status === 'late'
                      ? 'bg-amber-100 text-amber-700'
                      : 'bg-red-100 text-red-700',
                  ]"
                >
                  {{ activeJustification.status }}
                </span>
              </div>
              <p
                v-if="activeJustification.date"
                class="text-xs text-slate-400 mt-1"
              >
                Date: {{ activeJustification.date }}
              </p>
            </div>
          </div>
          <div class="text-right">
            <div
              class="flex items-center gap-2 text-emerald-600 bg-emerald-50 px-4 py-2 rounded-xl border border-emerald-100"
            >
              <ShieldCheck :size="18" />
        ...
        <div class="flex-1 overflow-y-auto p-10 space-y-8">
          <div class="space-y-4">
            <div class="flex items-center gap-3">
              <div
                class="size-8 bg-primary text-white rounded-lg flex items-center justify-center"
              >
            ...
            <div class="bg-primary/5 border border-primary/10 rounded-3xl p-8">
              <p
                class="text-lg text-slate-700 font-medium leading-relaxed italic"
              >
                "{{
                  activeJustification.educationComment ||
                  "No comment provided."
                }}"
              </p>
            </div>
      ...
      >
        <div
          class="size-32 bg-slate-50 rounded-[3rem] flex items-center justify-center text-slate-200"
        >
          <Bell :size="64" />
        </div>
        <div class="max-w-xs space-y-2">
          ...
          <p class="text-sm text-slate-400 font-medium">
            Choose an absence justification from the sidebar.
          </p>
        </div>
      </div>
    </div>
