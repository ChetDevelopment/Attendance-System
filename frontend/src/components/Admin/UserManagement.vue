...
const statusClass = (status: string) => [
  ...
  status === 'Active' ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400',
];

const powerClass = (status: string) => [
  'p-2 rounded-lg transition-all',
  status === 'Active' ? 'text-red-400 hover:bg-red-50' : 'text-green-400 hover:bg-green-50',
];
</script>
...
<template>
  <div class="space-y-6">
    ...
    <Modal :is-open="isCreateModalOpen" title="AdminLecturer - Staff Management" size="lg" @close="isCreateModalOpen = false">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="space-y-6 border-r border-slate-100 pr-8">
          <div class="relative">
            ...
            <div class="border border-slate-200 rounded-xl p-6 pt-8 space-y-4">
              <div class="grid grid-cols-3 items-center gap-4">
                ...
                <input v-model="newUser.id" type="text" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Title</label>
                <select v-model="newUser.title" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20">
              ...
              <div class="grid grid-cols-3 items-center gap-4">
                ...
                <input v-model="newUser.name" type="text" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Department</label>
                <select v-model="newUser.department" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20">
                  <option>Science</option><option>Arts</option><option>Engineering</option><option>Medicine</option>
                </select>
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Email</label>
                <input v-model="newUser.email" type="email" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Username</label>
                <input v-model="newUser.username" type="text" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Password</label>
                <input v-model="newUser.password" type="password" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">User Type</label>
                <select v-model="newUser.role" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20">
                  <option>Teacher</option><option>Staff</option><option>Admin</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div class="flex gap-3">
            <button @click="handleCreateUser" class="flex-1 py-2 bg-white border border-slate-300 rounded shadow-sm text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Save</button>
            <button class="flex-1 py-2 bg-white border border-slate-300 rounded shadow-sm text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Update</button>
            <button @click="isCreateModalOpen = false" class="flex-1 py-2 bg-white border border-slate-300 rounded shadow-sm text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Cancel</button>
          </div>
        </div>
        ...
        <div class="flex flex-col justify-between py-4">
          ...
          <div class="relative mt-8">
            <span class="absolute -top-3 left-4 bg-white px-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Edit Lecturer</span>
            <div class="border border-slate-200 rounded-xl p-6 pt-8 space-y-4">
              <div class="grid grid-cols-3 items-center gap-4">
                <label class="text-xs font-bold text-slate-600">Lecturer ID</label>
                <input type="text" class="col-span-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded text-sm outline-none focus:ring-2 focus:ring-primary/20" />
              </div>
              <div class="flex justify-end">
                <button class="px-6 py-1.5 bg-white border border-slate-300 rounded shadow-sm text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all">Edit</button>
              </div>
            </div>
          </div>
        </div>
